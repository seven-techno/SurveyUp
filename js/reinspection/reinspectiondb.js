...
function Edit(_id){
  //localStorage.setItem("reinspection",_id);
  window.location.href = 'reinspectionupdate.html?_id=' +_id;
//location.assign("reinspectionupdate.html");
}
function View(_id){
 //localStorage.setItem("reinspection",_id);
 window.location.href = 'viewreinspection.html?_id=' +_id;
 //location.assign("viewreinspection.html");
}

function clearFields(){
  $("#form_final_private_id")[0].reset();
}
function DisplayReinspection(){
   // location.assign("displayTable.html");
...
function display(data){
    var str="", str1="",str2="",str3="",str4="",str5="",count=1;
    for( var i=0;i<data.length;i++){
      char=data[i].doc._id;
       char=char.substring(17,34);
        //console.log(char);
       if(char=='REINSPECTION')
       {
       // console.log(Object.keys(vehicleListsTable));
        ...
        /*"</td><td>"+data[i].doc.company_address_final_private_id+
        "</td><td>"+data[i].doc.policy_number_code_final_private_id+"</td><td>"+data[i].doc.policy_number_final_private_id+
        "</td><td>"+data[i].doc.claim_number_code_final_private_id+"</td><td>"+data[i].doc.claim_number_final_private_id+
        "</td><td>"+data[i].doc.insurer_code_final_private_id+
        "</td><td>"+data[i].doc.insurer_name_final_private_id+"</td><td>"+data[i].doc.insurer_address_final_private_id+
        ...
        +data[i].doc.vehicle_registerd_number_final_private_id+"</td><td>"+data[i].doc.vehicle_registration_date_final_private_id+
        "</td><td>"+data[i].doc.vehicle_make_final_private_id+"</td><td>"+data[i].doc.vehicle_model_final_private_id+"</td><td>"
        +data[i].doc.vehicle_chassis_number_final_private_id+"</td><td>"+data[i].doc.vehicle_engine_number_final_private_id+
       ...
       "</td><td><table class=table table-striped table-hover table-bordered>"+
        ...
        + "badge no"+"</th></tr>";
        for(key in data[i].doc.vehicleListTable){
            var val=data[i].doc.vehicleListTable[key];
       ...
       str+= "</table></td><td>"+data[i].doc.dl_issuing_authority_final_private_id+"</td><td>"+data[i].doc.dl_type_final_private_id+
        ...
        "</td><td>"+data[i].doc.accident_date_final_private_id+
        "</td><td>"+data[i].doc.accident_time_final_private_id+"</td><td>"+data[i].doc.accident_place_final_private_id+
        "</td><td>"+data[i].doc.accident_cause_final_private_id+"</td><td>"+data[i].doc.property_damage_final_private_id+
        "</td><td>"+data[i].doc.details_of_injury_final_private_id+"</td><td>"
        +data[i].doc.accident_reported_to_police_final_private_id+"</td><td>"
        +data[i].doc.name_of_police_station_final_private_id+"</td><td>"+data[i].doc.fir_details_final_private_id+

        
         "</td><td>"+data[i].doc.survey_request_date_final_private_id+
        "</td><td><table class=table table-striped table-hover table-bordered>"+
        "<tr><th>"+"Sl.No"+"</th><th>"+"Date"+"</th><th>"+"Time"+"</th></tr>";
        for (key in data[i].doc.surveyTable){
            var val=data[i].doc.surveyTable[key];
            str+="<tr><td>"+key+"</td><td>"+val.date+"</td><td>"+val.time+"</td></tr>";
        }

       str+= "</table></td><td>"+data[i].doc.survey_place_final_private_id+
       "</td><td><table class=table table-striped table-hover table-bordered>"+
        "<tr><th>"+"Sl.No"+"</th><th>"+"Part name"+"</th><th>"+"Nature of damage"+"</th></tr>";
        for(key in data[i].doc.damagepartsTable){
            var val=data[i].doc.damagepartsTable[key];
            str+="<tr><td>"+key+"</td><td>"+val.part_name+"</td><td>"+val.nature_of_damage+"</td></tr>";
        }
       ...
       str+= "</table></td><td>"+data[i].doc.spot_survey_charges_final_private_id+
        ...
        "</td><td>"+data[i].doc.postage_charges_final_private_id+*/
         ...
         "</td><td><input type='button'  id='viewBtn' value='View' onClick='View(\""+data[i].doc._id+"\");'/>"+
          "</td><td><input type='button' id='deleteBtn' value='Delete' onClick='Delete(\""+data[i].doc._id+"\");'/></td></tr>";
        



        //str1+="<tr><td>"+data[i].doc._id+"</td><td>"+data[i].doc.rc_verification_selection_final_private_id+"</td><td>"
        //+data[i].doc.vehicle_registerd_number_final_private_id+"</td><td>"+data[i].doc.vehicle_registration_date_final_private_id+
        //"</td><td>"+data[i].doc.vehicle_make_final_private_id+"</td><td>"+data[i].doc.vehicle_model_final_private_id+"</td><td>"
        //+data[i].doc.vehicle_chassis_number_final_private_id+"</td><td>"+data[i].doc.vehicle_engine_number_final_private_id+
        //"</td><td>"+data[i].doc.vehicle_colour_final_private_id+"</td><td>"+data[i].doc.road_tax_selection_final_private_id+
       ...
       // "</td><td><input type='button' value='Remove' onClick='Delete(\""+data[i].doc._id+"\");'/></td></tr>";
        ...
        //str3+="<tr><td>"+data[i].doc._id+"</td><td>"+data[i].doc.accident_date_final_private_id+
        //"</td><td>"+data[i].doc.accident_time_final_private_id+"</td><td>"+data[i].doc.accident_place_final_private_id+
        //"</td><td>"+data[i].doc.accident_cause_final_private_id+"</td><td>"+data[i].doc.property_damage_final_private_id+
        //"</td><td>"+data[i].doc.details_of_injury_final_private_id+"</td><td>"
        //+data[i].doc.accident_reported_to_police_final_private_id+"</td><td>"
       ...
       // +data[i].doc.outstation_distance_travelled_final_private_id+"</td><td>"
        //+data[i].doc.number_of_photos_final_private_id+"</td><td>"+data[i].doc.per_photo_price_final_private_id+
       // "</td><td>"+data[i].doc.outstation_allowance_final_private_id+
        //"</td><td>"+data[i].doc.postage_charges_final_private_id+"</td><td><input type='button' value='Remove' onClick='Delete(\""+data[i].doc._id+"\");'/></td></tr>";
    }
    count++;
}
